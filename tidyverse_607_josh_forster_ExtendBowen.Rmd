...
### Extend: Andrew Bowen
Extending Josh's `tidyverse` code above. He did a great job of cleaning the ELO dataframe into a very usable format. I'd like to add some additional visualizations of the dataset


First up, let's take a look at the diftribution of `season_diff` values:
```{r}
ggplot(sum_stats, aes(x=season_diff)) + geom_histogram(binwidth=20)
```

This looks slightly skewed right. I'm a Bulls fan so I'd like to plot a time series of their
...
This is good but it's a little busy, let's group by week and get the average the Bull's 
```{r}
library(lubridate)
...
Interesting to see the jump in average ELO ratings between the 2020 and 2021 seasons. This coincides with when the Bulls made some free agent signings and upgraded their roster. Let's see if we can make a scatter plot of post-game vs pre-game ELO ratings for the Bulls, 
